...
export async function GET() {
  try {
    ...
    if (apiKeysError || !apiKeys || apiKeys.length === 0) {
      return NextResponse.json({ success: false, message: "No Bitmart API credentials found" }, { status: 400 })
    }
    ...
    if (!apiKey.enabled) {
      return NextResponse.json({ success: false, message: "Bitmart API key is disabled" }, { status: 400 })
    }
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: "Successfully connected to Bitmart API",
      ...
      if (apiError.message && apiError.message.includes("RATE_LIMITED")) {
        return NextResponse.json(
          { success: false, message: "Rate limited by Bitmart API. Please try again in a few minutes." },
          { status: 429 },
        )
      }
      ...
      return NextResponse.json(
        { success: false, message: apiError.message || "Failed to connect to Bitmart API" },
        { status: 500 },
      )
    }
  } catch (error: any) {
